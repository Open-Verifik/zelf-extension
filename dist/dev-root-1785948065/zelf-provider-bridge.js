...
"use strict";
/*!**********************************************************!*\
  !*** ./content-scripts/provider/zelf-provider-bridge.ts ***!
  \**********************************************************/
...
function setupMessageBridge() {
    window.addEventListener("message", async (event) => {
        if (event.source !== window)
            return;
        if (!event.data || event.data.source !== "zelf-inpage")
            return;
        const { type, payload, requestId } = event.data;
        if (!DAPP_MESSAGE_TYPES.includes(type))
            return;
