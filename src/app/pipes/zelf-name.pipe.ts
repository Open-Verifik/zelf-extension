...
export class ZelfNamePipe implements PipeTransform, OnDestroy {
    ...
    transform(value?: string, ...args: number[]): unknown {
        ...
        if (!isNaN(+args[0])) {
            ...
            if (windowWidth < 480 && zelfName?.trim().length > 12) {
                return `${zelfName.slice(0, 12 - 10)}...${zelfName.slice(-9)}`.toUpperCase();
            }

            if (windowWidth < 768 && zelfName?.trim().length > 22) {
                return `${zelfName.slice(0, 22 - 10)}...${zelfName.slice(-9)}`.toUpperCase();
            }
        }
