...
@Component({
    ...
    template: `
        ...
        >
            ...
            <div>
                <div class="hwc-hold-actions-text" *ngIf="wallet.publicData.expiresAt">
                    <small>
                        {{ "unlock_wallet.hold.expiration_time" | transloco }}
                    </small>
                    <small class="font-bold">
                        {{ wallet.publicData.expiresAt | date : "medium" }}
                    </small>
                </div>
            </div>
...
export class WalletCardComponent implements OnInit {
    ...
    async _syncDetails(): Promise<any> {
        ...
        const expiresAt = this.wallet.publicData.expiresAt;

        // check if it's expired
        this.wallet.publicData.isExpired = !expiresAt || new Date(expiresAt) < new Date();
        ...
        this.wallet.publicData.expiresAt =
            zelfNameObject.publicData.type === "hold" ? zelfNameObject.publicData.expiresAt : zelfNameObject.publicData.expiresAt;

        this.wallet.publicData.isExpired = !this.wallet.publicData.expiresAt || moment(this.wallet.publicData.expiresAt).isBefore(moment());
    }
